...
def register():
    ...
    new_user = User(username=data['username'], password_hashed=hashed_password)
...
def login():
    ...
    token = jwt.encode({
        'user_id': user.id,
        'exp': lambda: datetime.now(timezone.utc) + datetime.timedelta(hours=24) #token expires after 24 Hours
